...
function M.notify(message, level, opts)
  opts = opts or {}
  local ok = pcall(vim.notify, message, level or vim.log.levels.WARN, opts)
  if not ok then
    vim.schedule(function()
      vim.notify(message, level or vim.log.levels.WARN, opts)
    end)
  end
end
...
function M.command_availability(commands)
  ...
  return {
    ...
    available = available,
    missing = missing,
    message = #missing == 0 and "All required commands are available"
      or "Missing required commands: " .. table.concat(missing, ", "),
  }
end
